...
### Tutorial aims
- Understand How the Placement of Effects and How They are Combined (* or +) Changes What Your Model Means and Its Summary Output
- Decide When to Use Which Effect Placement and Whether to Multiply or Add
- Guage a Deeper Understanding of Model Building Through Performance Comparison by Result Interpretation
...
It is important to highlight that when modelling your own data, you should not try out different models and choose the one that is the best fit, as this leads to P-Hacking. However, in this tutorial, we are trying and testing the different models and comparing the outputs to demonstrate what each one means, and in what scenarios you would choose each one.

#### Why this dataset?
It is estimated that 30% of anthropogenic global greenhouse gas emissions are attributed to agriculture, with by far the largest proportion of that coming directly from raising and feeding livestock (Tubiello et al., 2013). Reducing our meat consumption is a vital, and feasible, component of reducing global warming, yet many countries meat consumption is still on the rise. This is why I chose to analyse this dataset, as I thought it would be interesting and meaningful to find the exact model outputs of how meat consumption in five major countries from different continents has changed since 1990, when controlling for the different types of meat.

### Some data set wrangling... (Boring but Necessary) 
#### Libraries and Datasets 
First we need to load the packages that we will be using in this tutorial. There are only three to load. Remember to install the packages if you have not done so before on the version of R that you are using. If you try to add a package to the library that you haven't installed it will give you the error message: 
- "Error in library(dplyr) : there is no package called 'dplyr'"

Make sure that you have already downloaded the dataset and know the directory of where it is saved. The data is available in the repository named "meat_consumption.csv"
...
```
The data appeared to be right-skewed, suggesting a gamma or negative binomial distribution. To clarify which one of the two it is we have to compare varience with mean abundance... A larger variance means that the data is overdispersed, and the data dispersion is negative binomial.
...
![Rplot_scatterplot](https://github.com/user-attachments/assets/99179669-50d1-43b9-8bf1-80e653dce3c4)

Figure 1 - A Scatter plot of scaled meat consumption over time (years) across our five different countries. Each point is colour-coded in relation to the type of meat (subject), noted in the key. Note that the scales are 'free' to give us an idea of how consumption rates are changing in each country, but this means that you cannot compare the graphs to each other. (This was required as some countries, eg Norway, comparatively had such low consumption rates the plot was unreadable) 
...
To begin with, we are just going to model the relationship between the predictor (scaled time) and the response (scaled value of meat consumption) variable alone. To do this, we only need to use a Generalised Linear Model (GLM) with no fixed or random effects. 
This model examines the effect of time on meat consumption, without considering that there are different types of meat and different countries, treating all datapoints as if they belong to a single population.
...
```
This model generates a single line with one intercept and one gradient, averaging the value for meat consumption over time for all countries and all types of meat (depicted in Figure 2). It assumes that the relationship between the value and time is constant across the board. However, we know that this isn't correct, because we have seen in our scatter plot (Figure 1) that meat consumption varies between countries, and between subjects. This means that this simple model overlooks at least two critical sources of variation.
In all of our models the response variable is value (of meat consumption) and the predictor variable is time (both of which are scaled)
mod_1 would be appropriate if you hypothesised a simple relationship between the response variable and one predictor without accounting for any additional factors or grouping factors. This is a rare scenario and is not often seen in ecology or real-world datasets, because relationships tend to be complex and influenced by multiple factors.
In summary, Model 1 does provide a basic starting point, but its simplicity makes it unsuitable for complex datasets, such as this one, where there are additional grouping factors that introduce variability.

![Rplot_Mod_1](https://github.com/user-attachments/assets/2292a79f-cccc-459b-96e2-670570f11c9c)

Figure 2 - An example of what data modelled using the simplicity of Model 1 could look like on a log-log scale. The data depicted is fictional and is not actual data from the dataset, as it is just to demonstrate the assumptions of the model.

#### Model 2 - General Linear Model with two predictors
This model is slightly more complex than Model 1, as it contains two predictor values, time and location, each of which have their own individual effect on the response variable. Like Model 1, this is still a General Linear Model and contains no mixed effects, but it does model the countries as fixed effects. This model accounts for time trends while recognising that meat consumption may vary by country.
...
``` 
This model uses '+ LOCATION' to add a fixed effect to our model. Because LOCATION is added rather than multiplied it allows each country to have a different 'intercept', reflecting differences in baseline mean consumption between countries (consumption at scaled_year 0, or 1990). However, by adding the fixed effect, the model assumes that each countries line will have the same gradient. This is depicted in Figure 3, where all five countries have parallel trend lines to each other (same gradient) but begin at different starting points (intercepts). 
You would use this model if you believe that your fixed effects will cause a variation in starting points (intercepts) but will share the same trend in relation to the predictor.

![Rplot_mod_2](https://github.com/user-attachments/assets/21f038aa-8c87-4d4f-a15d-d503c8f15430)


Figure 3 - An example of what data plotted using the assumptions of Model 2 could look like on a log-log scale. Each countries data has different intercepts, however they all have the same rate of change of meat consumption over time (gradient). The data depicted is fictional and is not actual data from the dataset, as it is just to demonstrate the assumptions of the model.

#### Model 3 - General Linear Model with a multiplied fixed effect
In model 2 the fixed effect 'LOCATION' was added to our model. In model three the same fixed effect is present, but this time it is multiplied to include an interaction term. The interaction term 'scaled_time * LOCATION' allows the model to estimate both different intercept and different gradients (slopes) for each country. This means that the effect of time on meat consumption can vary per country.
...
```
You would use a model with this format, containing a multiplicative interaction term, when you expect the effect of one predictor (in this case time) on the response variable (in this case meat consumption) to vary depending on the levels of another predictor (in this case country). Because 'LOCATION' is treated as a fixed effect (not random - we will learn more about these) the output of the model will tell us different values for each country, for both intercept and slope. We are telling the model that country affects meat consumption trends over time, and we want to know how it affects it, depending on the country.
...
Hurray! Onto our first Mixed Model! In mixed models, we can include both fixed and random effects. Random effects are used when you want to account for variability across groups or units, but you do not want to know in what way each group varies. This tells the model that there will be changes in the data between the groups that the random effect contains, and the model will account for this, but it will not tell you the variation within the output. A random effect, written as (1 | LOCATION) will allow variation in intercepts for each country's meat consumption (baseline consumption in 1990) but does not allow for varying gradients. This will model the data in the same way to a fixed effect that is added (rather than multiplied), except the output is different.
...
```
You would use random effects in this way when you know that the intercept varies across groups (in this case countries), but you don't want to know HOW it changes the intercept. The random effect allows the model to account for the variability, making the outputs more reliable, but it won't tell you each individual intercept for each country, like it would when LOCATION was a fixed effect (Model 2).

#### Model 5 - General Linear Mixed Model with fixed and random effects
Still reading..? Well done! We're onto our penultimate model, so things are getting pretty precise! This time we are going to have a fixed and a random effect within the same model. We are keeping our fixed effect as it was in Model 3, with a multiplicative interaction term (* LOCATION), but this time we are introducing variation due to the different types of meat, or 'SUBJECTS'. The random effect will be treated the same as in Model 4, but will be SUBJECT rather than LOCATION.
This means we can work out what each bit means, as we already have learnt what both of these interactions do. Multiplying the fixed effect into the model is saying that the meat consumption will vary across each country, both in the intercept and the gradient, and the output will tell us how this varies. And adding the random effect (1 | SUBJECT), will allow a variation of the intercept of each line for each SUBJECT, but we don't what to know how they will differ. The random effect assumes that all meat types share the same gradient over time (same rate of change), but the intercepts (baseline values) will differ between them.
```html
...
```
This model is asking: 'how does meat consumption change over time and how does this relationship differ across countries, while accounting for differences between meat types?'. It should be used if you want to know how one effect (country) changes the output and you don't want to know how the other effect (meat type) does, but you want to tell the model that there will be variation due to meat type.
...
### Challenge
Which TYPE of model (between 1-6) should be used for each of the following scenarios? Choose which model works best with the data you are presented with, and give an example piece of code for each scenario (ignore 'data = ' and assume a normal distribution)
